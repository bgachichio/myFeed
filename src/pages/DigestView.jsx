    ...
    <div className={`border rounded-2xl overflow-hidden ${colorClass}`}>
      <button onClick={() => setOpen(v => !v)} className="w-full flex items-center justify-between px-5 py-4 hover:opacity-80 transition-opacity">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs opacity-60">{localArticles.length} article{localArticles.length !== 1 ? 's' : ''}</span>
          {unread > 0 && <span className="text-xs font-medium px-1.5 py-0.5 bg-white/60 dark:bg-black/30 rounded-full">{unread} unread</span>}
        </div>
        {open ? <ChevronUp className="w-4 h-4 opacity-50" /> : <ChevronDown className="w-4 h-4 opacity-50" />}
      </button>

      {open && (
        <div className="bg-white dark:bg-stone-900 border-t border-current border-opacity-10 divide-y divide-stone-50 dark:divide-stone-800">
          {localArticles.map(article => (
            <div key={article.id} className={`px-5 py-3.5 flex items-start gap-3 group hover:bg-stone-50 dark:hover:bg-stone-800/50 transition-colors ${article.is_read ? 'opacity-60' : ''}`}>
              <div className="mt-1.5 flex-shrink-0">
                {!article.is_read ? <span className={`w-1.5 h-1.5 rounded-full block ${dotClass}`} /> : <span className="w-1.5 h-1.5 block" />}
              </div>
              <div className="flex-1 min-w-0">
                <button onClick={() => handleOpen(article)} className="text-left w-full">
                  <p className="text-sm font-medium text-stone-900 dark:text-stone-100 leading-snug hover:text-brand-700 dark:hover:text-brand-400 transition-colors mb-0.5">{article.title}</p>
                  {article.description && <p className="text-xs text-stone-500 dark:text-stone-400 leading-relaxed mb-1">{article.description}{article.description.length >= 278 ? '...' : ''}</p>}
                  <div className="flex items-center gap-2 text-xs text-stone-400 dark:text-stone-500">
                    <span className="font-medium">{article.feeds?.title}</span>
                    {article.pub_date && <><span>·</span><span className="flex items-center gap-1"><Clock className="w-3 h-3" />{formatPubDate(article.pub_date)}</span></>}
                  </div>
                </button>
              </div>
              <div className="flex items-center gap-1 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => handleBookmark(article)} className={`p-1.5 rounded-lg transition-colors ${article.is_bookmarked ? 'text-brand-600 bg-brand-50 dark:bg-brand-900/30' : 'text-stone-300 dark:text-stone-600 hover:text-stone-600 dark:hover:text-stone-300 hover:bg-stone-100 dark:hover:bg-stone-700'}`}>
                  {article.is_bookmarked ? <BookmarkCheck className="w-3.5 h-3.5" /> : <Bookmark className="w-3.5 h-3.5" />}
                </button>
                <button onClick={() => handleOpen(article)} className="p-1.5 rounded-lg text-stone-300 dark:text-stone-600 hover:text-stone-600 dark:hover:text-stone-300 hover:bg-stone-100 dark:hover:bg-stone-700 transition-colors">
...
export default function DigestView() {
  ...
  useEffect(() => { loadDigest() }, [dateRange])
  ...
  const loadDigest = async () => {
    ...
    } catch { /* silent — digest falls back to empty state */ }
  ...
  return (
    <div>
      ...
      {!loading && totalArticles > 0 && (
        <div className="flex items-center gap-4 mb-6 p-4 bg-white dark:bg-stone-900 border border-stone-100 dark:border-stone-800 rounded-xl">
          ...
          {[...Array(3)].map((_, i) => (
            <div key={i} className="bg-white dark:bg-stone-900 border border-stone-100 dark:border-stone-800 rounded-2xl p-5 animate-pulse">
              ...
              <div className="space-y-3">
                {[...Array(3)].map((_, j) => <div key={j} className="h-4 bg-stone-50 dark:bg-stone-800/50 rounded w-3/4" />)}
              </div>
            </div>
          ))}
        </div>
