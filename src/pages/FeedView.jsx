...
export default function FeedView() {
  ...
  const loadArticles = useCallback(async (category = 'All', newOffset = 0, append = false, rf = 'unread') => {
    try {
      const result = await getArticles(user.id, { category, limit: PAGE_SIZE, offset: newOffset, readFilter: rf })
  ...
  useEffect(() => {
    ...
    Promise.all([loadArticles(activeCategory, 0, false, readFilter), loadCategories()])
  ...
  const handleLoadMore = async () => {
    ...
    setLoadingMore(true)
    try { await loadArticles(activeCategory, offset, true, readFilter) }
  ...
  const handleCategoryChange = async (cat) => {
    ...
    try { await loadArticles(cat, 0, false, readFilter) }
    finally { setLoading(false) }
  }
  ...
  const handleReadFilterChange = async (rf) => {
    setReadFilter(rf)
    setLoading(true); setOffset(0); setSelectedIds(new Set())
    try { await loadArticles(activeCategory, 0, false, rf) }
    finally { setLoading(false) }
  }
    ...
    try {
      ...
      results.forEach((result, i) => {
        ...
        if (result.status === 'fulfilled') {
          result.value.items.slice(0, 10).forEach(item => allArticles.push({
          ...
          updateFeedHealth(feed.id, { success: true, articleCount: result.value.items.length })
        } else {
          updateFeedHealth(feed.id, { success: false, error: result.reason?.message })
        }
      })
